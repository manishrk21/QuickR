...
// 1. Normal: Supabase session cookie (admin dashboard)
// 2. Test:   X-Service-Key header matching SUPABASE_SERVICE_ROLE_KEY (k6 load tests)
...
export async function POST(req: NextRequest) {
  ...
  // ── Auth mode 1: service key header (for k6 load tests) ──────────────────
  const serviceKeyHeader = req.headers.get("x-service-key");
  const isServiceKeyAuth =
    serviceKeyHeader &&
    serviceKeyHeader === process.env.SUPABASE_SERVICE_ROLE_KEY;

  if (!isServiceKeyAuth) {
    // ── Auth mode 2: normal Supabase session cookie (admin dashboard) ───────
  ...
  // For service key auth, trust the restaurant_id in the body
  // For session auth, verify the restaurant_id matches the session
  if (!isServiceKeyAuth && restaurantId !== restaurant_id) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  ...
  if (mobile) {
    const { data: customer } = await adminSupabase
      .from("customers")
      .upsert(
        { mobile, name: customer_name ?? null, restaurant_id, is_guest: false },
...
// old working above one is for testing 
