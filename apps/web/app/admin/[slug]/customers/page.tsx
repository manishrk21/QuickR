...
export default async function CustomersPage({
  ...
  const rpcIds = Array.isArray(rpcCustomers) ? rpcCustomers.map((customer: any) => customer?.id).filter(Boolean) : [];
  ...
  const candidateCustomers = rpcError || rpcIds.length === 0 ? rows : rows.filter((customer: any) => rpcIds.includes(customer.id));
  const effectiveRows = candidateCustomers.length > 0 ? candidateCustomers : rows;

  if (effectiveRows.length > 0) {
    const customerIds = effectiveRows.map((customer: any) => customer.id);
    ...
    customers = effectiveRows.map((customer: any) => {
