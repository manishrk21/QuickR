...
export function MenuItemCard({ item, primaryColor }: any) {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-16 w-16 shrink-0 flex items-center justify-center bg-slate-100/50 rounded-lg">
        {item.signedImageUrl ? (
          <img
            src={item.signedImageUrl}
            alt={item.name}
            className={`h-full w-full object-cover rounded-lg border border-slate-100 transition-all duration-500 ease-out origin-center ${
              isFocused 
                ? "scale-125 -translate-y-1 shadow-lg border-slate-200/80 z-10" 
                : "scale-100 translate-y-0 shadow-sm"
            }`}
          />
        ) : (
          <div className="flex h-full w-full items-center justify-center bg-slate-100 text-[9px] font-medium text-slate-400 rounded-lg border border-slate-100">
            No Image
          </div>
        )}
      </div>
